...
def test(args):
    ...
    Gab = define_Gen(input_nc=3, output_nc=3, ngf=args.ngf, netG='dpn_gen', norm=args.norm, 
                                                    use_dropout= not args.no_dropout, gpu_ids=args.gpu_ids)
    Gba = define_Gen(input_nc=3, output_nc=3, ngf=args.ngf, netG='dpn_gen', norm=args.norm, 
                                                    use_dropout= not args.no_dropout, gpu_ids=args.gpu_ids)

    utils.print_networks([Gab,Gba], ['Gab','Gba'])

    try:
        ckpt = utils.load_checkpoint('%s/DPN_saved_weights.ckpt' % (args.checkpoint_dir))
    ...
    """ run """
    for i in range(10):         # A20551677 Generate 10 images
        ...
        torchvision.utils.save_image(pic, args.results_dir+'/sample'+str(i)+'.jpg', nrow=2)
        torchvision.utils.save_image(a_real_test, args.results_dir+'/a_real'+str(i)+'.jpg', nrow=2)   # Ruthwik Adapala storing all these images to calculate PSNR and SSIM from these 50 generated images. (treating this as test)
        torchvision.utils.save_image(b_fake_test, args.results_dir+'/a_gen'+str(i)+'.jpg', nrow=2)
        torchvision.utils.save_image(b_real_test, args.results_dir+'/b_real'+str(i)+'.jpg', nrow=2)
        torchvision.utils.save_image(a_fake_test, args.results_dir+'/b_gen'+str(i)+'.jpg', nrow=2)

...
class cycleGAN(object):
    ...
    def train(self,args):
        ...
        b_loader = torch.utils.data.DataLoader(dsets.ImageFolder(dataset_dirs['trainB'], transform=transform), 
                                                        batch_size=args.batch_size, shuffle=True, num_workers=4)
        # a = iter(a_loader)
        # im, t = next(a)          # Ruthwik Adapala made changes to check the shape 
        # print(im.shape)
        ...
        for epoch in range(self.start_epoch, args.epochs):
            ...
            for i, (a_real, b_real) in enumerate(zip(a_loader, b_loader)):
                ...
                a_idt = self.Gab(a_real)           # Ruthwik Adapala added Identity loss to improve the results
                b_idt = self.Gba(b_real)       
                                                         
                #print(a_fake.shape, b_fake.shape, a_idt.shape, b_idt.shape, a_real.shape, b_real.shape)
                # Identity losses
                ###################################################
                a_idt_loss = self.L1(a_idt, a_real) * args.lamda * args.idt_coef            # Ruthwik Adapala changed hyperparameter in tuning identity loss
                ...
                ###################################################
                gen_loss = a_gen_loss + b_gen_loss + a_cycle_loss + b_cycle_loss + a_idt_loss + b_idt_loss    # A20551677 changed total loss to incorporate identity loss. 
